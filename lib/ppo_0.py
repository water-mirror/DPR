...
class Net_Actor(nn.Module):
    ...
    def forward(self, data):
        ...
        gat_1_out = self.gat_n(gat_0_out, e_n)
        gat_2_out = self.gat_1(gat_1_out, e_r1)
        gat_3_out = self.gat_1(gat_2_out, e_r1)
        gat_4_out = self.gat_n(gat_3_out, e_n)
        gat_5_out = self.gat_n(gat_4_out, e_n)
        gat_6_out = self.gat_2(gat_5_out, e_r0)
        gat_7_out = self.gat_2(gat_6_out, e_r0)
        gat_x_out = self.gat_n(gat_7_out, e_n)
        x = gat_x_out.reshape((-1, self.n_nodes, self.node_dim))
        x = x * h
        
        scores = self.lin_prob(x[:,1:])
        probs = torch.squeeze(F.softmax(scores, dim=-1), -1)
        distrib_p = Categorical(probs)
        
        sisr_params_mean = torch.squeeze(self.lin_sisr_mean(x[:,1:]),-1)
        sisr_params_std = torch.squeeze(torch.abs(self.lin_sisr_std(x[:,1:])),-1)
        distrib_s = Normal(sisr_params_mean, sisr_params_std)
        h = torch.squeeze(h,1)
...
class ActorCritic(nn.Module):
    ...
    def act(self, data, memory):
        h, distrib_p, distrib_s = self.actor(data)
        action_node = distrib_p.sample()
        action_sisr = distrib_s.sample().detach()
#         print(action_sisr, distrib_s.log_prob(action_sisr))
        coeff = (torch.exp(action_sisr)+torch.exp(-action_sisr)-2).cpu()
        coeff *=torch.from_numpy(((action_sisr.cpu().detach().numpy()>0)*2-1).astype(np.float32))
        coeff = torch.sigmoid(coeff)
        return h, action_node, action_sisr, coeff, distrib_p, distrib_s
    
    def evaluate(self, memory_state, memory_node, memory_sisr):
        h, distrib_p, distrib_s = self.actor(memory_state)
        action_logprobs_1 = distrib_p.log_prob(memory_node)
        action_logprobs_2 = distrib_s.log_prob(memory_sisr)
        action_logprobs = action_logprobs_1 * action_logprobs_2[torch.arange(action_logprobs_2.size(0)), memory_node]
        dist_entropy = distrib_p.entropy() + distrib_s.entropy()[torch.arange(action_logprobs_2.size(0)), memory_node]
    ...
    def update(self, memory):
        ...
        for _ in range(self.K_epochs):
            for batch in loader:
                ...
                logprobs, state_v, dist_entropy = self.policy.evaluate(batch, batch.actions_node, tmp_batch_sisr)
